...
import BorrowOperationsAbi from '../../../../core/abi/liquity/BorrowOperation.json';
...
import { LiquityProviderConfig } from '../../configs/liquity';
import { DefiDateData } from '../../types';
import DefiProvider, { GetDefiDateDataProps } from '../defi';

class LiquityProvider extends DefiProvider {
  public readonly name: string = 'provider.liquity';

  constructor(configs: LiquityProviderConfig) {
    super(configs);
  }
  ...
  public async getDateData(props: GetDefiDateDataProps): Promise<DefiDateData> {
    ...
    const events = await eventCollection
      .find({
        contract: normalizeAddress(this.configs.lendingContract),
        timestamp: {
          $gte: date,
          $lt: date + 24 * 60 * 60,
        },
    ...
    const borrowOperationContract = new web3.eth.Contract(BorrowOperationsAbi as any, this.configs.lendingContract);
