...
import CompoundLendAbi from '../../../../core/abi/compound/LendToken.json';
import envConfig from '../../../../core/env';
import { getHistoryTokenPriceFromCoingecko, normalizeAddress } from '../../../../core/helper';
import logger from '../../../../core/logger';
import { CompoundLendingConfig } from '../../configs/compound';
import { DefiDateData } from '../../types';
import DefiProvider, { GetDefiDateDataProps } from '../defi';
import { getPoolConfigByAddress } from './helpers';

class CompoundProvider extends DefiProvider {
  public readonly name: string = 'provider.compound';

  constructor(configs: CompoundLendingConfig) {
    super(configs);
  }

  public async getDateData(props: GetDefiDateDataProps): Promise<DefiDateData> {
    const { date, providers } = props;

    const dateData: DefiDateData = {
      module: 'defi',
    ...
    for (let poolId = 0; poolId < this.configs.pools.length; poolId++) {
      const events = await eventCollection
        .find({
          contract: normalizeAddress(this.configs.pools[poolId].poolAddress),
          timestamp: {
            $gte: date,
            $lt: date + 24 * 60 * 60,
          },
      ...
      const poolConfig = getPoolConfigByAddress(this.configs.pools[poolId].poolAddress, this.configs.pools);
      if (!poolConfig) {
        logger.onDebug({
          source: this.name,
      ...
      if (historyPrices[poolConfig.underlyingCoingeckoId]) {
        historyPrice = historyPrices[poolConfig.underlyingCoingeckoId];
      } else {
        historyPrice = await getHistoryTokenPriceFromCoingecko(poolConfig.underlyingCoingeckoId, events[0].timestamp);
        historyPrices[poolConfig.underlyingCoingeckoId] = historyPrice;
      }
